...
void HelloWorld::randomPlanet() {
    ...
    for(int i = 0; i < 5; ){
		float x = std::rand()%100000/100000.*(winSize.width-planetRadius*2)+planetRadius;
		float y = std::rand()%100000/100000.*(winSize.height-planetRadius*2)+planetRadius;
        int dir = std::rand()%2;
        if(dir == 1)
			x = winSize.width-x;
        dir = std::rand()%2;
...
bool HelloWorld::init()
{
	...
	//CCDirector::sharedDirector()->getOpenGLView()->setDesignResolutionSize(visibleSize.width, visibleSize.height, kResolutionNoBorder);
    ...
    if(state == 1) {
        if(countTime >= thinkTime) {
            ...
            if(bluePlanets->count() > 0 && redPlanets->count() > 0) {
                int randBlue = std::rand()%bluePlanets->count();
